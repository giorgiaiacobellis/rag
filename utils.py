...
def load_data_in_batches(filename, app):
    ...
    for i in range(0, len(data), BATCH_SIZE):
        ...
        formatted_batch = [
            {"text": item["text"]} for item in batch  # Estrai il campo "text"
        ]
...
def chat_with_chatbot(config, session):
    ...
    while True:
        try:
            ...
            answer = app.chat(input_query, session_id=session, citations=False)  # Assumiamo che 'app' sia definito correttamente
...
def evaluate_model(config, evaluator, embedding, test_questions, test_answers, filename="evaluation_results.json"):
    ...
    ds = generate_responses(app, test_questions, test_answers) #genera le risposte del modello
